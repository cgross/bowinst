...
var writeBowinstJs = function(index, setupjs){

    var tpl = fs.readFileSync(path.join(__dirname,'..','bowinstjs.tpl'),'utf8');
    tpl = _.template(tpl,{index:index,setupjs:setupjs});
...
var doWrites = function(promptData){
    ...
    if (promptData.index !== 'index.html' || promptData.setupjs !== 'js/setup.js') {
        writeBowinstJs(promptData.index,promptData.setupjs);
    }
...
module.exports = function(){
    ...
    prompt.get([indexPrompt,setupJsPrompt],function(err,result){
        ...
        if ((result.index !== 'index.html' || result.setupjs !== 'js/setup.js') && fs.existsSync(bowinstjsPath)) {
