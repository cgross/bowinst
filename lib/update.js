...
module.exports = function(name){
	...
	added.forEach(function(file){
        ...
        console.log(chalk.bold(template) + ' installed in ' + chalk.bold(config.fileTypes[ext].file) + '.');
	...
	removed.forEach(function(file){
        ...
        parsed.lines = _(parsed.lines).filter(function(line){
            if (line.indexOf(componentFile) !== -1){
                console.log(chalk.bold(line.trim()) + ' uninstalled from ' + chalk.bold(config.fileTypes[ext].file) + '.');
                return false;
            }
            return true;
        });
