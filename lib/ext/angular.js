    ...
    install: function(bowerjson,options){
        ...
        if (!bowerjson.angularModule){
            ...
            if (_(bowerjson.dependencies).find(function(value,key){ return key === 'angular' || key === 'angular-unstable'; })){
                console.log(bowerjson.name.bold + ' appears to be an Angular module but does NOT include an ' + 'angularModule'.bold + ' property.  You\'ll likely need to add the module dependency manually.');
            }
    ...
    update: function(oldBowerjson,newBowerjson,options){
        ...
        if (neww && old){
            parsed.modules = _.map(parsed.modules,function(mod){
                if (mod === old){
                    console.log('Angular module reference changed from ' + old.bold + ' to ' + neww.bold + ' in ' + options.file.bold + '.');
                    return neww;
                }
                return mod;
            });
